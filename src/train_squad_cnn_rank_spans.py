...
def evaluate_lenet5(learning_rate=0.01, n_epochs=2000, batch_size=100, emb_size=300, char_emb_size=20, hidden_size=300,
                    ...
                    char_filter_size=4, margin=0.85, max_EM=50.302743615):
    ...
    # id2char = {y:x for x,y in char2id.iteritems()}
    # word2vec=load_glove()
    # rand_values=load_word2vec_to_init(rand_values, id2word, word2vec)
...
#     index = T.lscalar()
    ...
    NN_para=[conv_W_1, conv_b_1,conv_W_2, conv_b_2,conv_W_char, conv_b_char]
...
#     test_input4score = T.concatenate([test_span_reps, T.repeat(q_rep.dimshuffle(0,1,'x'), test_gram_size, axis=2)], axis=1) #(batch, 2*hidden, 5*p_len_limit-(0+1+2+3+4))
    ...
    input4score = squad_cnn_rank_spans(rng, common_input_p, common_input_q, char_common_input_p, char_common_input_q,batch_size, p_len_limit,q_len_limit, 
                         ...
                         conv_W_1, conv_b_1,conv_W_2, conv_b_2,conv_W_char,conv_b_char,
